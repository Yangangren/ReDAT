...
class Recorder(object):
    def __init__(self):
        self.val2record = ['v_x', 'v_y', 'r', 'x', 'y', 'phi', 'front_wheel', 'a_x',
                           'delta_y', 'delta_v', 'delta_phi',  'cal_time',
                           'ref_index', 'beta', 'path_values', 'ss_time', 'is_ss', 'delta_steer', 'delta_a_x', 'light']
        self.val2plot = ['v_x', 'r', 'front_wheel', 'a_x', 'cal_time', 'ref_index', 'beta', 'path_values', 'is_ss', 'delta_steer', 'delta_a_x']
        self.key2label = dict(v_x='Speed [m/s]',
                              r='Yaw rate [rad/s]',
                              front_wheel='Front wheel angle [$\circ$]',
                              a_x='Acceleration [$\mathrm {m/s^2}$]',
                              cal_time='Computing time [ms]',
                              ref_index='Selected route',
                              beta='Side slip angle[$\circ$]',
                              path_values='Path value',
                              is_ss='Safety shield',
                              delta_steer='Delta wheel',
                              delta_a_x='Delta acc')
    ...
    def record(self, obs, act, cal_time, ref_index, path_values, ss_time, is_ss, lights):
        ego_info, tracking_info = obs[:self.ego_info_dim], \
                                  obs[self.ego_info_dim:self.ego_info_dim + self.track_info_dim]

        v_x, v_y, r, x, y, phi, steer, a_x = ego_info
        delta_lon, delta_lat, delta_phi, delta_v = tracking_info
        delta_steer, delta_a_x = act[0], act[1]

        # transformation
        beta = 0 if v_x == 0 else np.arctan(v_y/v_x) * 180 / math.pi
        self.val_list_for_an_episode.append(np.array([v_x, v_y, r, x, y, phi, steer, a_x, delta_lat,
                                        delta_phi, delta_v, cal_time, ref_index, beta, path_values, ss_time, is_ss, delta_steer, delta_a_x, lights]))
