...
class ReplayBufferWithAttention(object):
    ...
    def add(self, obs, act, reward, obs_tp1, done, future_n_point, mask, future_n_edge):
        data = (obs, act, reward, obs_tp1, done, future_n_point, mask, future_n_edge)
    ...
    def _encode_sample(self, idxes):
        obses, acts, rewards, obses_tp1, dones, future_n_points, masks, future_n_edges = [], [], [], [], [], [], [], []
        for i in idxes:
            data = self._storage[i]
            obs, act, reward, obs_tp1, done, future_n_point, mask, future_n_edge = data
            ...
            future_n_edges.append(future_n_edge)
        ...
        return np.array(obses, dtype=np.float32), np.array(acts, dtype=np.float32),\
               ...
               np.array(masks, dtype=np.float32), np.array(future_n_edges, dtype=np.float32)
