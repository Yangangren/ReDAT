...
config = dict(
        ...
        pred_hori=2,  # prediction horizon
...
class Veh_obj:
    ...
    def update_risk(self, **kwargs):
        ...
        if 'obj_name' in kwargs.keys():
            ...
            flag = 0
        for key, val in kwargs.items():
            exec("self.{0}.append(val)".format(key), {'self': self, 'val': val})  # for ego: record the risks to each obj
            if flag == 1: 
        ...
        for i in range(t_step + 1):
            ...
            for _n in range(len(ego.future_position)):
                for _m in range(len(obj.future_position)):
                    d_man = np.abs(ego.future_position[_n][0][i] - obj.future_position[_m][0][i]) + np.abs(ego.future_position[_n][1][i] - obj.future_position[_m][1][i])
                    if d_man < d_min:
                        d_min = d_man
                        rec = (_n, _m)
            dis_t.append(np.clip(np.sqrt((ego.future_position[rec[0]][0][i] - obj.future_position[rec[1]][0][i])**2 + (ego.future_position[rec[0]][1][i] - obj.future_position[rec[1]][1][i])**2) - (ego.width + obj.width)/2, 0 ,None))
    else:
        raise KeyError
...
def podar_render(frame: PODAR, ego_name: str = None):
    ...
    c_norm = colors.Normalize(vmin=0, vmax=1.0, clip=True)
    ...
    for _name, _veh in {**frame.ego, **frame.obj}.items():
        rec_handle = _draw_rotate_rec(_veh, 'black' if _name != ego_name else 'red', 'white' if _name == ego_name else mycmap(c_norm(frame.ego[ego_name].risk2obj[_name])))
        ...
        print('id={:<4} x={:<10.2f} y={:<10.2f} v={:<6.2f} phi={:<8.2f} risk={:<8.5f} l={:<6.1f} w={:<6.1f}'.format(_veh.name, _veh.x0, _veh.y0, _veh.speed, _veh.phi0/np.pi * 180,
                    frame.ego[ego_name].risk2obj[_name] if _name != ego_name else np.max(frame.ego[ego_name].risk),
